...
memos_to_replace = {
    ...
    "Pagto Cobranca Lanche&amp;co Alimentos Ltda - me": "Pagto Cobranca Lancheco",
    "Pagto Cobranca Lancheco Alimentos Ltda": "Pagto Cobranca Lancheco",
}
...
class OrderedDefaultdict(OrderedDict):  # name according to default
    ...
    def __missing__(self, key):
        self[key] = value = self.__default_type()  # change to whatever default you want
        return value
...
class StatementItem(object):
    ...
    def __eq__(self, other):
        return (
            self.id == other.id
            and self.trn_type == other.trn_type
    ...
    def __repr__(self):
        return "StatementItem({}/{}/{}/{}/{})".format(
            self.id, self.trn_type, self.date, self.amount, self.memo
        )
...
class XlsxHelper(object):
    ...
    def _set_formats(self):
        ...
        self.money = self.workbook.add_format(
    ...
    def add_comment(self, comment, lines=None):
        if lines is None:
            ...
            self.worksheet.write_comment(
                self.row, self.col, comment, self.comment_format
            )
...
class Statement(object):
    ...
    def to_xls_per_item(self, workbook):
        ...
        for description in self.description_per_month.keys():
            ...
            for period in self.description_per_month[description].keys():
                ...
                workbook.write(
                    row_desc, column_per_period[period], value, workbook.money
                )
    ...
    def to_xlsx_grouped(self, workbook, target_file_name):
        def set_printing_options():
            ...
            worksheet.set_footer(
                "&L" + target_file_name + "&C" + statement_date + "&RPage &P of &N"
            )
        ...
        def add_descricao_total():
            ...
            workbook.worksheet.write_rich_string(
                workbook.row,
                workbook.col,
                "Total Crédito: ",
                credit_color,
                "{:15,.2f}".format(soma_credito),
                " Débito: ",
                debit_color,
                "{:15,.2f}".format(soma_debito),
                " Diferenca: ",
                delta_color,
                "{:15,.2f}".format(delta),
            )  # descricao
        ...
        descricao = "Descricao ({}/{}/{})".format(
            self.bank_id, self.account_id, self.account_type
        )
    ...
    def to_csv_mensal(self):
        ...
        output += forma.format(
            ...
            lf=lf,
            periodo="total",
            entrada=self.total.total_credit,
            saida=self.total.total_debit,
            delta=self.total.delta,
        )
    ...
    def to_csv_grouped(self):
        ...
        output = "Período{sep}Descrição{sep}Quantidade{sep}Crédito{sep}Débito{sep}%{sep}{sep}Soma Crédito{sep}Soma Débito{sep}Diferença{lf}".format(
            lf=lf, sep=sep
        )
        ...
        for period_name in self.periods:
            ...
            for name in sorted(statement_period.credits):
                ...
                output += forma.format(
                    ...
                    lf=lf,
                    periodo=period_name,
                    descricao=fixed_name,
                    quantidade=statement_memo.count,
                    debito="",
                    credito=statement_memo.total,
                    percent="",
                )
            ...
            for name in sorted(statement_period.debits):
                ...
                output += forma.format(
                    ...
                    quantidade=statement_memo.count,
                    credito="",
                    debito=statement_memo.total,
                    percent=percent,
                )

            delta = abs(soma_credito) - abs(soma_debito)
            descricao = '"Total Crédito: {:15,.2f} Débito: {:15,.2f} Diferença: {:15,.2f}"'.format(
                soma_credito, soma_debito, delta
            )
            output += forma_final.format(
                sep=sep,
                lf=lf,
                periodo=period_name,
                descricao=descricao,
...
class MasterStatement(object):
    ...
    def add_data(self, data):
        ...
        for item in statements:
            ...
            for (
                memo_to_only_care_about_the_prefix
            ) in memos_to_only_care_about_the_prefix:
                ...
                if statement_item == self.statement_ids[statement_item.id]:
                    ...
                    print(
                        "ERROR: Invalid OFX. Two different items with the same key:\n\t{}\n\t{}".format(
                            statement_item, self.statement_ids[statement_item.id]
                        )
                    )
                    raise (
                        ValueError(
            ...
            self.inputs["MEMO_{}".format(statement_item.trn_type)][
                statement_item.memo
            ] += 1
            ...
            self.output.description_per_month[statement_item.memo][
                statement_item.get_period()
                ...
                if self.bank_account_from[key] != target.get(key, None):
                    raise ValueError(
                        "Source and Target Bank Accounts are not the same: {} / {} / {}".format(
                            key, self.bank_account_from[key], target.get(key, None)
                        )
                    )
...
if len(sys.argv) < 2:
    print(f"usage: {sys.argv[0]} SOURCE_OFX_FILES [--debug]")
    print(f"usage: {sys.argv[0]} SOURCE_JSON_FILES [--debug]")
    sys.exit(1)
...
if "--debug" in sys.argv:
    save_target_file(
        target_file_name, "_processed.json", master_statement.output.to_json()
    )
    save_target_file(
        target_file_name, "_mensal.csv", master_statement.output.to_csv_mensal()
    )
    save_target_file(
        target_file_name, "_grouped.csv", master_statement.output.to_csv_grouped()
    )
